...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending) {
        ...
        for (int i = 0; i < n - 1; i++) { // where's the min or max
            int selected = i;

            for (int j = i + 1; j < n; j++) { // iterates through unsorted
                if (ascending) { // ascending order to find smaller than selected
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        Arrays.sort(asteroids); // sort

        for (int i = 0; i < asteroids.length; i++) { // go through each asteroid
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        int left = 0; // p1
        int right = people.length - 1; // p2
        int sleds = 0;

        while (left <= right) {
            if (people[left] + people[right] <= limit) { // light with heaviest
                left++;
            }
            right--;
            sleds++;
